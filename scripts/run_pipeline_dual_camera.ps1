...
function Merge-DualCameraModel {
    ...
    $camerasContent = Get-Content $camerasFile -Raw
    $imagesContent = Get-Content $imagesFile
    $points3DContent = Get-Content $points3DFile -Raw

    # Parse existing camera (camera 1 = W)
    # Format: CAMERA_ID, MODEL, WIDTH, HEIGHT, PARAMS[]
    $cameraLines = $camerasContent -split "`n" | Where-Object { $_ -and $_ -notmatch "^#" }
    ...
    # Get W camera info
    $wCameraLine = $cameraLines[0]
    $wCameraParts = $wCameraLine -split "\s+"
    ...
    $imageLines = $imagesContent | Where-Object { $_ -and $_ -notmatch "^#" }
